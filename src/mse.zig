...
pub fn MeanSquaredError(comptime T: type) type {
    return struct {
        ...
        pub fn computeLoss(self: Self, prediction: Matrix(T), labels: Matrix(T)) T {
            assert(mem.eql(usize, &prediction.shape, &labels.shape));
...
test "Compute the loss" {
    ...
    const a = Matrix(f32).fromBuffer(.{ 1, 3 }, &a_data);

    var b_data = [_]f32{ 0.0, 1.0, 0.0 };
    const b = Matrix(f32).fromBuffer(.{ 1, 3 }, &b_data);

    const mse = try MeanSquaredError(f32).init(tst.allocator, 3);
    defer mse.deinit();

    const loss = mse.computeLoss(a, b);

    try tst.expectEqual(loss, 11.0 / 3.0);
}
...
test "Compute gradient of the loss with respect to the input" {
    ...
    var mse = try MeanSquaredError(f32).init(tst.allocator, 3);
    defer mse.deinit();

    _ = mse.computeLoss(a, b);
    const grad = mse.computeGradient(a, b);

    try tst.expectEqualSlices(f32, grad.elements, &.{ -2.0, -2.0, -6.0 });
}
